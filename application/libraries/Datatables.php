...
class Datatables
{
    ...
    private function get_total_results($filtering = FALSE)
    {
        ...
        if (count($this->group_by) > 0) {
            ...
            $this->ci->db->get($this->table, NULL, NULL, FALSE);
            $query = $this->ci->db->last_query();
            $count = $this->ci->db->query('select count(total) as jumlah from (' . $query . ') as queryCount');
            return $count->row()->jumlah;
        }
        $this->ci->db->select('count(1) as jumlah');
        $count = $this->ci->db->get($this->table, NULL, NULL, FALSE);
        return $count->row()->jumlah;
    }
